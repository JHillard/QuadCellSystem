...
SIGNAL = OPTICAL_POWER* responsivity;
T = 273 %Kelvin
...
OUTPUT = SIGNAL * noise_tf;
NOISE = (v_noise.^2 + i_noise.^2) .^0.5;
figure
loglog(w,NOISE,'.');
hold on;
loglog(w,v_noise);
loglog(w,i_noise);
legend('Total Noise', 'Voltage Noise', 'Current Noise');
title('Total TIA noise');
xlabel('Hz');
ylabel('V per rt Hz');
%loglog(OUTPUT);
get_rms(NOISE,df)
